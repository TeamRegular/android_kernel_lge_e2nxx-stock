...
#ifdef CONFIG_LGE_PM_EXT_OVP_FET_CTRL
...
struct qpnp_chg_regulator {
	...
	|| defined (CONFIG_MACH_MSM8226_T8WIFI) || defined (CONFIG_MACH_MSM8226_T8WI7U)
int count_count = 0 ;
...
/* Charging information Debugging Log */
    ...
    defined (CONFIG_MACH_MSM8926_T8LTE) || defined (CONFIG_MACH_MSM8926_E8LTE) || defined (CONFIG_MACH_MSM8226_T8WIFI) || defined (CONFIG_MACH_MSM8226_T8WI7U)/* || \
...
#ifdef CONFIG_LGE_PM_EXT_OVP_FET_CTRL
#ifdef CONFIG_CHG_DETECTOR_MAX14656
	struct delayed_work	ext_ovp_fet_work;
#endif
#endif
};
...
#elif defined (CONFIG_MACH_MSM8226_E7WIFI) || defined (CONFIG_MACH_MSM8926_T8LTE) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
...
/*              */
    ...
    defined (CONFIG_MACH_MSM8926_E8LTE) || defined (CONFIG_MACH_MSM8926_T8LTE) || defined (CONFIG_MACH_MSM8926_E8LTE_KR) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
...
#define FACTORY_IBAT_MAX_FOR_EMBEDDED_BATTERY 100
    ...
    defined (CONFIG_MACH_MSM8926_T8LTE) || defined (CONFIG_MACH_MSM8926_E8LTE_KR) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
...
#elif defined (CONFIG_MACH_MSM8226_T8WIFI) || defined (CONFIG_MACH_MSM8226_T8WI7U)
		gpio_direction_output(ext_ovp_gpio, on);
		pr_err("external ovp set value %d\n",on);
#endif
...
qpnp_chg_vbatdet_lo_irq_handler(int irq, void *_chip)
{
    ...
    defined (CONFIG_MACH_MSM8926_E8LTE_KR) || defined(CONFIG_MACH_MSM8226_T8WIFI) || \
    defined(CONFIG_MACH_MSM8226_T8WI7U) || defined (CONFIG_MACH_MSM8926_X10_VZW)
		...
		if (!usb_present) {
			...
			if (lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO)
				...
				if (booting_withTA == 1)
					...
					schedule_delayed_work(&chip->input_current_check_work, HZ*5);
			}
			chip->is_vzw_slow_charging = false;
#endif
		}
...
{
	struct qpnp_chg_chip *chip = _chip;
#if defined(CONFIG_MACH_MSM8926_E8LTE_KR) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
	...
	if(!is_factory_cable()){
		...
		if((boot_mode == LGE_BOOT_MODE_QEM_56K)
				|| (boot_mode == LGE_BOOT_MODE_CHARGERLOGO)) {
		//In case of MiniOS booting or chargerlogo
		//	with Normal cable or without cable
#ifdef CONFIG_LGE_PM_CHARGING_EXTERNAL_OVP
			// DCP Cable
...
qpnp_aicl_check_work(struct work_struct *work)
{
	...
	if (!charger_monitor) {
		if (aicl_check_count < 15) /* 150 seconds at 10 second intervals */
...
#if defined (CONFIG_MACH_MSM8226_E7WIFI) || defined (CONFIG_MACH_MSM8226_E8WIFI) || \
    defined (CONFIG_MACH_MSM8926_E8LTE) || defined (CONFIG_MACH_MSM8226_E9WIFI) || \
    defined (CONFIG_MACH_MSM8226_E9WIFIN) || defined (CONFIG_MACH_MSM8926_T8LTE) || defined (CONFIG_MACH_MSM8926_E8LTE_KR) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
int
...
#if defined (CONFIG_MACH_MSM8226_T8WIFI) || defined (CONFIG_MACH_MSM8226_T8WI7U)
	if (chgr_sts & TRKL_CHG_ON_IRQ)
#else
...
static void vzw_fast_chg_input_current_check_work(struct work_struct *work)
{
	...
	if (!ext_ovp_present) {
		if (iusb_max <= threshold && (get_prop_capacity(chip) < 100)
...
extern unsigned int g_usb_ta_status;
#endif
static void
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT)
	g_usb_ta_status = ret.intval | chip->ac_online;
#endif
    #endif
...
#ifndef CONFIG_LGE_PM_PMIC_8x26_CHARGER
...
#ifdef CONFIG_LGE_PM_EXT_OVP_FET_CTRL
static void ext_ovp_fet_work_func(struct work_struct *work) {
	int boot_mode = lge_get_boot_mode();

	struct delayed_work		*dwork = to_delayed_work(work);
	struct qpnp_chg_chip	*chip = container_of(dwork,
			struct qpnp_chg_chip, ext_ovp_fet_work);
#ifdef CONFIG_CHG_DETECTOR_MAX14656
	union power_supply_propval ret = {0,};

	if (!chip->detect_psy)
		chip->detect_psy = power_supply_get_by_name("max14656");

	chip->detect_psy->get_property(chip->detect_psy,
			POWER_SUPPLY_PROP_USB_CHG_TYPE, &ret);

	pr_err("boot_mode %d, fastchg_on %d, max14656_chg_type %d, usb %d\n",
			boot_mode, qpnp_chg_is_fastchg_on(chip), ret.intval,
			qpnp_chg_is_usb_chg_plugged_in(chip));
#else
	pr_err("boot_mode %d, fastchg_on %d, ac_online %d, usb %d\n",
			boot_mode, qpnp_chg_is_fastchg_on(chip), chip->ac_online,
			qpnp_chg_is_usb_chg_plugged_in(chip));
#endif
	if (boot_mode == LGE_BOOT_MODE_CHARGERLOGO) {
		if (qpnp_chg_is_fastchg_on(chip)) {
#ifdef CONFIG_CHG_DETECTOR_MAX14656
			if (ret.intval == USB_DCP_CHARGER && qpnp_chg_is_usb_chg_plugged_in(chip)) {
#else
			if (chip->ac_online && qpnp_chg_is_usb_chg_plugged_in(chip)) {
#endif
				lge_set_chg_path_to_external();
				pr_err("DCP CABLE - Ext. OVP is HIGH\n");
			}
		}
	}
}
...
*/
    ...
    defined (CONFIG_MACH_MSM8926_E8LTE) || defined (CONFIG_MACH_MSM8226_E9WIFI) || \
    defined (CONFIG_MACH_MSM8226_E9WIFIN) || defined (CONFIG_MACH_MSM8926_E9LTE) || \
    defined (CONFIG_MACH_MSM8926_T8LTE) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U) || defined(CONFIG_MACH_MSM8926_E8LTE_KR)
...
#elif defined (CONFIG_MACH_MSM8226_T8WIFI) || defined (CONFIG_MACH_MSM8226_T8WI7U)
			gpio_request(chip->ext_ovp_gpio, "pm_ext_ovp_ctrl");
#endif
			break;
		case SMBB_DC_CHGPTH_SUBTYPE:
...
#endif
#ifdef CONFIG_LGE_PM_EXT_OVP_FET_CTRL
	INIT_DELAYED_WORK(&chip->ext_ovp_fet_work, ext_ovp_fet_work_func);
	schedule_delayed_work(&chip->ext_ovp_fet_work,
...
static int qpnp_chg_suspend(struct device *dev)
{
    ...
    defined (CONFIG_MACH_MSM8926_E8LTE) || defined (CONFIG_MACH_MSM8226_E9WIFI) || \
    defined (CONFIG_MACH_MSM8226_E9WIFIN) || defined (CONFIG_MACH_MSM8926_E9LTE) || \
    defined (CONFIG_MACH_MSM8926_T8LTE) || defined(CONFIG_MACH_MSM8226_T8WIFI) || defined(CONFIG_MACH_MSM8226_T8WI7U)
	if (chip->ac_online && chip->chg_done) {
